...
type typ =
  ...
  | TAlpha of talpha
      (** 'a would be stored as TAlpha "a" *)
  | TArrow   of typ * typ
      (** t1 -> t2 would be stored as TArrow (t1,t2) *)
  | TStar    of typ * typ
      (** t1 * t2 would be stored as TStar (t1,t2) *)
  | TVariant of typ list * tname
      (** 'a list would be stored as TVariant ([TAlpha "a"], "list") *)

(** a record storing all the information about a variant type. 
 * For example, if define the variant ['a list = Nil | Cons of 'a * 'a list]
 * it will be represented as 
 * [{vars = ["a"];
 *   name = "list";
 *   constructors = [("Nil",TUnit); ("Cons",TStar(TAlpha "a", TVariant([TAlpha "a"];"list")))]
 *  }]
 *)
type variant_spec = {
  vars: talpha list;
